...
estas estrategias no han mostrado ser eficientes, debido al alto costo
 ...
 $\Omega\subseteq\cup_{s=1}^{N_{c}}\Omega_s$. 
 ...
 \begin{figure}[h!]
  ...
  \caption{a) El dominio $\Omega$ es dividido en subdominios de forma qué 
   $\Omega=\cup_{s=1}^{N_{c}}\Omega_s$.
   b) Luego, cada subdominio $\Omega_s$ es extendido  
  en dos puntos (círculos grises), los cuales se superponen con los subdominios vecinos, 
  de forma qué $\Omega\subseteq\cup_{s=1}^{N_{c}}\Omega_s$. 
  En esta región se recibe información de subdominios vecinos.
   c) Intercambio de información en los bordes de los subdominios solapados 
    $\Omega_s$ y $\Omega_{s-1}$.
   Exchange of boundaries at limiting edges of the overlapping subdomains $\Omega_s$ and $\Omega_{s-1}$. El 
   procesador $s$ envía a su vecino $s-1$ la información del tercer y cuarto puntos de la grilla, 
   y recibe de su vecino los primeros dos puntos en la grilla $s$.}
 \label{fig:bechange}
\end{figure}
 ...
 en la grilla dados por $x_s(i)=x_{\min,s}+(i-1)\Delta x$ y $y_s(j)=y_{\min,s}+(j-1)\Delta y$. 
...
\begin{algorithm}
\caption{Algoritmo FC--DOM en paralelo}\label{algfc}
\begin{algorithmic}[1]
\State  Generar una descomposición de dominio
\State Asignar un subdominio $\Omega_s$ a cada procesador.
\State \textbf{para} cada $(x,y) \in \Omega_s$ \textbf{hacer} en paralelo
\State \hskip0.5em  Dar vectores iniciales para iniciar el esquema de Adams--Bashforth.
...
\State \hskip1.0em Intercambiar bordes.
...
&\frac{1}{c}\frac{\partial }{\partial t}\ut + \hth \cdot \nabla \ut+a(\x)\ut  \\
&\quad \quad \quad  \quad \quad \; \;\,  +b(\x)\ut = \int_{S^1}\eta(\hth\cdot\hth ') 
u(\x,\hat \theta',t) d\theta'+s(\x,\hth,t),  \; (\x,\theta)  \in \Omega\times S^1\\
&u(\x,\theta,t=0)=u_0, \; (\x,\theta)  \in \Omega\times S^1,  \\
&u(\x,\theta,t)=u_b, \; (\x,\theta) \in \Gamma_-. \notag
\end{aligned}
\end{split}
\label{eq:RTEdisp}
\end{equation*}
...
\begin{equation*}
u^{\text{an}}(\x,\theta,t)=e^{-(x-t)^2-(y-t)^2-\cos(\theta)^2}.
\label{eq:RTEmansol}
\end{equation*}
...
\begin{wrapfigure}{r}{0.45\textwidth}
 ...
 \end{subfigure}
  \caption{Convergencia del algorítmo FC--DOM en paralelo 
  para cada una de las variables en la ETR. La figura 
  se muestra en escala logarítmica. Las lineas rectas discontinuas 
  muestran la pendiente para un órden de convergencia prescripto. 
  El algorítmo presenta convergencia espectral en la variable angular $\theta$.}
 \label{fig:convman}
\end{wrapfigure}
...
En esta sección comparamos los resultados producidos por el algorítmo FC--DOM 
...
%\section{Consecuencias numéricas de la capa límite en dos dimensiones espaciales}
