...
En el problema inverso, alguno de los parámetros son conocidos, 
...
En el contexto de esta Tesis, reconstruiremos, para distintos problemas, 
 el coeficiente de absorción $a(\x)$ en tomografía óptica. Esto encuentra numerosas aplicaciones.  
 La reconstrucción de las propiedades de absorción 
...
humano para la obtención de imágenes en medicina.
...
$b(\x)$. Esta se obtiene, generalmente, mediante técnicas de 
 imágenes de alta resolución~\cite{Althobaiti2017,Guven2003}, \eg Resonancia Magnética. 
  El uso de estas técnicas posee ciertas limitaciones. 
...
\begin{wrapfigure}{l}{0.48\textwidth}
  ...
  \caption{Esquema  MOBIR. El modelo directo es utilizado para obtener los observables simulados. 
  Se evalúa el error entre los datos experimentales y los simulados por medio de la función objetivo. 
  Luego, utilizando un método de minimización, se actualiza el coeficiente $a(\x)^{i+1}$ a 
  ser utilizado en la iteración subsiguiente, hasta converger. }
 \label{fig:mobir}
\end{wrapfigure}

En esta Tesis desarrollamos un algoritmo tipo MOBIR~\cite{Hielscher1999,Kim2010} (del inglés, {\em Model Based Iterative Image  
...
no lineal. A partir de un coeficiente de absorción $a(\x)^0$ inicial, 
...
con la variable $\x$ discretizada, $\alpha^i$ es el largo del paso de Newton (que lo consideraremos 
en general $\alpha^i=1$). En el caso del método de Newton (o {\em steepest descent}), la dirección de 
descenso vendrá dada por el gradiente $\mathbf{d}=-\nabla_a g$, donde la función objetivo $g$ será definida posteriormente. 
...
\label{sec:BFGS}
Siguiendo a Nocedal~\cite{Nocedal2006}, el método BFGS parte de considerar la expansión de 
...
Además, si $g$ es al menos dos veces diferenciable se cumple que 
\begin{equation}
...
\label{eq:direccion}
\end{equation}
El principal obstáculo para la aplicación de la dirección de Newton es la 
necesidad de calcular la inversa del Hessiano $\nabla_a^2 g^i$ de la función objetivo,  
cálculo que en tomografía óptica puede 
...
es actualizada a cada paso de la iteración. Formalmente, se suma y resta el término $ \nabla_a^2 g \mathbf{d}^i$ 
en la ecuación~\eqref{eq:Taylor2}, y se llega a
\begin{equation}
...
es $o(||\mathbf{a}^{i+1}-\mathbf{a}^{i}||)$. Tomando $\mathbf{d}^i=\mathbf{a}^{i+1}-\mathbf{a}^{i}$ se llega a la expresión
\begin{equation}
\begin{split}
\begin{aligned}
...
La inversa del Hessiano es aproximada exigiendo que se cumpla la última relación en~\eqref{eq:Taylor4}, que puede escribirse 
\begin{equation}
(B^{i+1})^{-1}y^i=s^i,
\label{eq:Taylor5}
\end{equation}
...
por cierto conocimiento previo, como por ejemplo, la imagen 
de resonancia magnética de un cuello humano utilizada en la sec.~\ref{sec:inverseres}.
Las cotas inferiores $a^l(\x)$ y superiores $a^u(\x)$ al coeficiente de absorción 
...
conocido se denominan inclusiones. En aquellos tejidos donde 
no ocurren inclusiones, el coeficiente de absorción tiene 
cotas superiores conocidas 
