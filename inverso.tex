...
\label{algfcinv}
\begin{algorithmic}[1]
\State \textbf{para} las iteraciones $i=1,\ldots,i_{\text{max}}$ \textbf{hacer}
...
\State \hskip0.5em Llamar al algoritmo lm-BFGS-b para actualizar el coeficiente $a^i(\x)$.       
\State  \textbf{terminar}
\end{algorithmic}
\end{algorithm}
...
adicionales consistieron en no considerar los vasos sanguíneos, y en la 
simplificación de la geometría del cuello, el cual se tomo como una geometría 
cuadrada.

Por otra parte, fijamos el valor  $a^l(\x)=a^u(\x)=a_b(\x)$ 
...
convergencia en el problema inverso de reconstrucción, y previene 
la amplificación de los errores numéricos, originados por la existencia 
de las capa límite~\cite{Gaggioli2021} discutidas en la sección~\ref{sec:}. 
La resolución de la capa límite en el problema multidimensional exige de métodos 
numéricos mas sofisticados que escapan al alcance de esta tesis. 

El valor de fondo para la imagen MRI es tomado como el coeficiente inicial 
en la iteración del método lm-BFGS-b. 

\begin{figure}[h!]
\centering
  \includegraphics[width=0.3\linewidth]{figuras/neck_mri.png} 
  \includegraphics[width=0.48\linewidth]{figuras/neck_i.png} 
  \caption{A la izquierda se tiene la Imagen de Resonancia Magnética~\cite{CCommons} para el modelo de cuello humano empleado. A la derecha, el coeficiente de absorción 
  generado a partir de esta imagen, el cual fue utilizado como el coeficiente 
  inicial en las reconstrucciones del coeficiente de absorción para éste modelo. 
