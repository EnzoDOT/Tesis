...
\centering
  ...
  \caption{De arriba hacia abajo: 
  Función $f(x)=3x^2-2.5x^3+0.5$, con los intervalos $[0,\delta_{\ell}]$ (verde) 
   $[1-\delta_r,1]$ (rojo) considerados
  para realizar las continuaciones. Continuaciones a cero a izquierda ($f_{\ell}(x)$) y a derecha ($f_r(x)$)
  obtenidas a partir de la proyección de los $d_{\ell}=d_r=5$ puntos (en diamantes). 
  Función continuada resultante, $f_c(x)$, de período $b=1.52$.}
